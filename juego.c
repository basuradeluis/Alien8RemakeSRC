...
//******************************************************************************
// Función decrementer_a_luz(char n)
//    Calcula las nuevas posiciones de los números del marcador de años luz.
//******************************************************************************
void decrementar_a_luz(char n)
{
 pos_a_luz[n]--;
...
//******************************************************************************
// Función dibujar_a_luz()
//    Dibuja los números del marcador de años luz.
//******************************************************************************
void dibujar_a_luz(void)
{
 int f;
...
//******************************************************************************
// Función cargar_hab()
//    Carga en Isomot la habitación indicada por la variable h.
//******************************************************************************
void cargar_hab(void)
 {
  int x,y;
...
// Se colocan los objetos
  ...
  for(f=(h?hab[h-1].n:0);f<hab[h].n;f++)    //v2:Importante: ver comienzo de ini_juego. Offset objeto inicial y offset del obj final
   {int z=ENCIMA;
    ...
    else if(mapa_obj[f] == RBALA) // Robot alienígena de movimiento libre
     {
      if(num_movs>=MAX_MOVS) num_movs=MAX_MOVS-1;
      if(num_obst>=MAX_OBST) num_obst=MAX_OBST-1;
      ism_obtener_coords_libres(rx,ry,&x,&y);
    ...
    else if(mapa_obj[f] <= RBALY) // Robot alienígena de movimiento fijo en x o y
     {
      if(num_movs>=MAX_MOVS) num_movs=MAX_MOVS-1;
      if(num_obst>=MAX_OBST) num_obst=MAX_OBST-1;
      ism_obtener_coords_libres(rx,ry,&x,&y);
...
//******************************************************************************
// Función mov_transportar(...)
//    Función de movimiento de los cubos que mueven los objetos que tienen
//    encima.
//    m0: índice del último (se encarga del sonido).
//    m1: en el último, indica si hay que reproducir el sonido.
//    g: en el último, canal del sonido.
//******************************************************************************
void mov_transportar(char f)
{
 char n;
 ...
 if(movs[0].anclado==movs[f].id)
  {
   movs[movs[f].m0].m1=1;
   // Únicamente muevo a Nacho si no ha colisionado.
   if(movs[0].sentmov<2)
 ...
 // Si es el último, gestiono el sonido
 if(f==movs[f].m0)
  {
   if(movs[f].m1)
    {
...
//******************************************************************************
// Función mov_z_pisado(...)
//    Función de movimiento de los cubos que bajan al pisarlos.
//******************************************************************************
void mov_z_pisado(char f)
{
...
//******************************************************************************
// Función mov_z_autom(...)
//    Función de movimiento de los cubos que se mueven en el eje z.
//******************************************************************************
void mov_z_autom(char f)
{
 char n,i=-1;
...
//******************************************************************************
// Función mov_zmn_pisado(...)
//    Función de movimiento de los cubos que suben y bajan al pisarlos.
//******************************************************************************
void mov_zmn_pisado(char f)
{
...
//******************************************************************************
// Función mov_xy_autom(...)
//    Función de movimiento de los cubos que se mueven en los ejes x o y.
//******************************************************************************
void mov_xy_autom(char f)
{
 char n;
 ...
 // Si he llegado a alguno de los límites, lo cambio de sentido.
 ...
 // Si lo consigo mover, muevo también los anclados.
...
//******************************************************************************
// Función mov_joystick(...)
//    Función de movimiento del joystick controlador del telebot.
//******************************************************************************
void mov_joystick(char f)
{
 char ac0,ac1,n;
...
//******************************************************************************
// Función mov_desaparece(...)
//    Función de movimiento del cubo que desaparece al pisarlo.
//******************************************************************************
void mov_desaparece(char f)
{
...
//******************************************************************************
// Función mov_mina_voladora(...)
//    Función de movimiento de las minas voladoras.
//******************************************************************************
void mov_mina_voladora(char f)
{
...
//******************************************************************************
// Función mov_mina_terrestre(...)
//    Función de movimiento de las minas terrestres.
//******************************************************************************
void mov_mina_terrestre(char f)
{
...
//******************************************************************************
// Función mov_telebot(...)
//    Función de movimiento del robot teledirigido.
//******************************************************************************
void mov_telebot(char f)
{
 static char mover=0;
       ...
       if(mover)
        {
         ...
         if(!ism_cambiar_dato_objeto(movs[f].id, movs[f].sentmov, movs[f].dir, SUMAR))
          {
           for(n=0;n<num_movs;n++) // Si tiene algún objeto anclado, lo movemos también.
            {
             if(movs[f].id==movs[n].anclado)
               if(ism_cambiar_dato_objeto(movs[n].id, movs[f].sentmov, movs[f].dir, SUMAR))
...
//******************************************************************************
// Función mov_rbal_l(...)
//    Función de movimiento del robot alienígena (dalek) de movimiento libre.
//    m0: temporizador de fotogramas.
//    m1: indicador de secuencia de giro.
//    sentmov: dirección de movimiento [0,1,2,3].
//    dir: fotograma.
//******************************************************************************
void mov_rbal_l(char f)
{
...
//******************************************************************************
// Función mov_rbal_f(...)
//    Función de movimiento del robot alienígena (dalek) de movimiento fijo.
//    m0: temporizador de fotogramas.
//    m1: indicador de secuencia de giro.
//    sentmov: dirección de movimiento [0,1,2,3].
//    dir: fotograma.
...
//******************************************************************************
// Función mov_saturno(...)
//    Función de movimiento del enemigo saturno.
//******************************************************************************
void mov_saturno(char f)
{
 int xs,ys,xn,yn;
...
//******************************************************************************
// Función mov_plasma(...)
//    Función de movimiento del enemigo plasma.
//******************************************************************************
void mov_plasma(char f)
{
 char n,i;
 ...
 if(movs[f].id!=NO_ID)
  {
   ...
   if(movs[f].g<1)
    {
     if(ism_cambiar_dato_objeto(movs[f].id, D_Z, movs[f].g/2-1, SUMAR))
      {// Ha chocado con algo mientras caía.
 ...
 if(movs[0].mover==mov_nacho_andar)
  {
   ...
   if(colision)
    {unsigned char cx;
     // Comprobamos si se sale de la habitación
     movs[0].sentmov=3;
...
void mov_nacho_saltar(char f)
{
 ...
 if(movs[0].mover==mov_nacho_saltar)
  {// No ha chocado con nada.
   ...
   // Avanzo en la dirección del salto
 ...
 for(f=n=0;f<num_movs;f++)
   if(movs[f].empujable)
     if(ism_colisionado_con(movs[f].id))
       n++;
 if(n==colision) // Únicamente empujo si todos los objetos con los que he colisionado son empujables.
  {// Buscamos un objeto empujable con el que haya colisionado.
   ...
   if(movs[f].mover==mov_mina_terrestre)
    { // Activo la explosión de la mina.
     movs[f].m0=1;
     if(i==0) mov_nacho_explotar(1);
    }
   else if(f!=0 || movs[0].sentmov<2)
    { // A Nacho únicamente se le empuja si no ha colisionado.
     ...
     if(colision)
      ...
      {// Si tiene algo anclado, lo muevo también.
       if(sonido_empujar==0)
        {
...
char anclar(char f)
{
 ...
 do
  {
   // Busco entre los que tienen función de movilidad.
   for(n=0;n<num_movs;n++)
     if(movs[n].id==id)
      {
       movs[f].anclado=id;
...
void ini_partida(void)
{
 ...
 pos_a_luz[0]=6*18;
 pos_a_luz[1]=pos_a_luz[2]=pos_a_luz[3]=10*18; // 600'0 años luz (una hora, dado que cada año luz equivale a seis segundos)
 dec_pos_a_luz=0;
